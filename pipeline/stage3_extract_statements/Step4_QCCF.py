#!/usr/bin/env python3
"""
Step 4: QC Cash Flow Extractions (Combined Formula + Source Match)

Validates CF extractions via:
1. Formula validation - evaluates Ref column formulas (G=A+B+C)
...
Input:  data/extracted_cf/*.md
        ...
        artifacts/stage3/qc_cf_accepted_exceptions.json (optional)
Output: artifacts/stage3/step4_cf_qc_results.json

Usage:
    python3 Step4_QCCF.py                    # QC all
    python3 Step4_QCCF.py --ticker LUCK      # Single ticker
    python3 Step4_QCCF.py --verbose          # Show all details
"""
...
EXCEPTIONS_FILE = PROJECT_ROOT / "artifacts" / "stage3" / "qc_cf_accepted_exceptions.json"
OUTPUT_FILE = PROJECT_ROOT / "artifacts" / "stage3" / "step4_cf_qc_results.json"

# Tolerances
FORMULA_TOLERANCE_PCT = 5.0  # 5% for formula validation (allows for line item placement issues)
...
def parse_table(content: str) -> tuple[list[dict], list[str]]:
    ...
    for line in content.split('\n'):
        ...
        # Remove only leading/trailing empty cells from pipe boundaries, preserve middle empty cells
...
def evaluate_formula(formula: str, refs: dict, period_idx: int) -> float | None:
    """Evaluate a formula like 'A+B+C' or 'sum(A:G)' using the refs dict."""
    formula = formula.strip()

    # Handle sum(X:Y) range syntax
    sum_match = re.match(r'sum\(([A-Z]):([A-Z])\)', formula, re.IGNORECASE)
    if sum_match:
        start_ref = sum_match.group(1).upper()
        end_ref = sum_match.group(2).upper()

        # Get all refs between start and end (alphabetically)
        total = 0
        in_range = False
...
def get_source_pages(extraction_path: Path, statement_pages: dict) -> list[Path]:
    """Get source markdown pages for a CF extraction."""
    ...
    if ticker in statement_pages:
        ...
        if period_key in ticker_data:
            ...
            if consolidation in period_data:
                cf_pages = period_data[consolidation].get('CF', [])
                for page_num in cf_pages:
...
def validate_file(filepath: Path, statement_pages: dict, verbose: bool = False) -> dict:
    """Validate a single CF extraction file (formula + source match)."""
...
def main():
    parser = argparse.ArgumentParser(description="QC Cash Flow extractions (formula + source match)")
    parser.add_argument("--ticker", help="Process only this ticker")
    parser.add_argument("--verbose", action="store_true", help="Show detailed output")
    args = parser.parse_args()

    print("=" * 70)
    print("STEP 4: QC CASH FLOW EXTRACTIONS (FORMULA + SOURCE MATCH)")
    print("=" * 70)
