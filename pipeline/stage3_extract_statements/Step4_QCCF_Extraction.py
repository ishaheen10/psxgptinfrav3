#!/usr/bin/env python3
"""
Step 4: QC Cash Flow Extractions (Pre-JSONify)
...
Output: artifacts/stage3/step4_qc_cf_extraction.json

Usage:
    python3 Step4_QCCF_Extraction.py                # Process all
    python3 Step4_QCCF_Extraction.py --ticker LUCK  # Single ticker
    python3 Step4_QCCF_Extraction.py --verbose      # Show details
"""
...
OUTPUT_FILE = PROJECT_ROOT / "artifacts" / "stage3" / "step4_qc_cf_extraction.json"
...
def parse_extraction_file(filepath: Path) -> dict:
    """
    Parse a CF extraction .md file.

    CF format: | Source Item | Canonical | Ref | 12M Jun 2024 | 12M Jun 2023 | ...

    Returns:
        {
            'columns': ['12M Jun 2024', '12M Jun 2023'],
            'rows': [
                {
                    'source': 'Cash from operations',
                    'canonical': 'cfo_operating',
                    'ref': 'A',
                    'formula': None,
...
def check_column_structure(parsed: dict) -> dict:
    ...
    Returns:
        {
            'has_issues': bool,
            'issues': [{'column': '12M Jun 2023', 'type': 'empty_column', 'fill_rate': 0.0}, ...]
        }
    """
    result = {
        'has_issues': False,
...
def main():
    parser = argparse.ArgumentParser(description="QC CF Extractions (Pre-JSONify)")
    parser.add_argument("--ticker", help="Process single ticker")
    parser.add_argument("--verbose", "-v", action="store_true", help="Show details")
    args = parser.parse_args()

    print("=" * 70)
    print("STEP 4: QC CASH FLOW EXTRACTIONS")
    print("=" * 70)
    print()
