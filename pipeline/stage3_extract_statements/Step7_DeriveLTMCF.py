#!/usr/bin/env python3
"""
Step 7: Derive LTM (Trailing Twelve Months) Cash Flow
...
It is stricter than P&L: every component must have direct provenance.

Input:  data/json_cf/*.json
Output: data/ltm_cf/*.json
        artifacts/stage3/step7_ltm_cf_qc.json
"""
...
OUTPUT_DIR = PROJECT_ROOT / "data" / "ltm_cf"
QC_OUTPUT = PROJECT_ROOT / "artifacts" / "stage3" / "step7_ltm_cf_qc.json"
...
def normalize_value(value: float, unit_type: str) -> float | None:
    if value is None:
        return None
...
def normalize_period(period: dict) -> dict:
    ...
    normalized = {}
    for canonical, raw_value in values.items():
        normalized[canonical] = normalize_value(raw_value, unit_type)
    out = dict(period)
    ...
    result = {
        ...
        "source_components": {
            "current": {
                ...
                "source_file": current.get("source_file") or current.get("source_filing"),
            ...
            "prior_annual": {
                ...
                "source_file": prior_annual.get("source_file") or prior_annual.get("source_filing"),
            ...
            "prior_same_duration": {
                ...
                "source_file": prior_same.get("source_file") or prior_same.get("source_filing"),
...
def main():
    parser = argparse.ArgumentParser(description="Derive LTM cash flow periods from reported cumulative periods")
    ...
    print(f"Generated LTM cash flow for {len(qc_results)} tickers")
