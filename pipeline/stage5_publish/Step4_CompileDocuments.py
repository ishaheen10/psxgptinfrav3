#!/usr/bin/env python3
"""
Step 4: Compile enriched JSONL database from markdown sources.
...
Output: database_jsonl_compiled/<TICKER>/<YEAR>/<FILING>.jsonl

Usage:
    python pipeline/stage5_publish/Step4_CompileDocuments.py
"""
...
def load_skip_page_map(path: Path) -> Set[str]:
    """Load skip manifest (JSON with 'pages' array or JSONL)."""
    if not path.exists():
        return set()
    skipped: Set[str] = set()
    ...
    # Try JSON format first (stage1 skip manifest)
    try:
        data = json.loads(content)
        if isinstance(data, dict) and "pages" in data:
            for page in data["pages"]:
                rel = page.get("relative_path") or page.get("path")
...
def main() -> None:
    print("=" * 60)
    print("STEP 4: COMPILE DOCUMENTS")
    print("=" * 60)
    ...
    print(f"Loaded {len(ticker_meta)} tickers from metadata")

    skip_pages = load_skip_page_map(SKIP_MANIFEST)
    print(f"Loaded {len(skip_pages)} pages to skip")

    section_tags = load_section_tags(TAGS_MANIFEST)
    print(f"Loaded section tags for {len(section_tags)} pages")
    print()
    ...
    print("COMPLETE")
    print("=" * 60)
    print(f"Tickers processed: {ticker_count}")
    print(f"Filings compiled:  {filing_count}")
